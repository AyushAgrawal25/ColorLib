...
myApp.controller('qbCtrl', function($scope,$interval) {
    ...
    $scope.menuClickFun=function(){
        if(angular.element(menuContent).hasClass("menu-open"))
        {
            $scope.menuCloseFun();
        }
        else
	...
	var qbSetSliderImgsFun=function(){
		var imgWrapperEle=angular.element(document.querySelector(".img-wrapper"));
		var imgWrapperWidth=(100*8/getImgEleNumFun())+"%";
		imgWrapperEle.css("width",imgWrapperWidth);
	}

	$scope.qbChangeImgFun=function(dotId){
		qbActivateImgFun(parseInt(dotId));
		qbActivateDotFun(parseInt(dotId));
	}

	var qbActivateDotFun=function(dotId){
		angular.forEach(dotEles,function(value,key){
			if(dotId==key)
			{
				angular.element(value).addClass("dot-active");
			}
			else
			{
				angular.element(value).removeClass("dot-active");
			}
		})
	}
	...
	var imgEleForwardFun=function(diff){
		...
		var changeInterval=window.setInterval(intervalFun,300);
